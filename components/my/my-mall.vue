<template>
	<view class="my-box">
		...
		<view v-if="userMode == 'panel'" class="user-panel-box">
			<!-- 已登录 -->
			<view class="u-flex" @click="openPage('my/user/user')">
		...
		<!-- 订单 -->
		<view v-if="orderMode == 'panel'" class="panel-box">
			<view class="panel-header"><u-section title="我的订单" sub-title="查看全部订单" :bold="false" @click="openPage('my/order/order')"></u-section></view>
			<u-grid :col="5" :border="false" class="panel-main">
				<u-grid-item @click="openPage('my/order/order')">
		...
		<view v-if="orderMode == 'card'" class="card-box">
			<u-grid :col="5" :border="false">
				...
				<u-grid-item class="all-order" @click="openPage('my/order/order')">
					<image class="cut-off-line" src="/static/user/cut_off_line.png" mode=""></image>
					<u-icon name="order" label="我的订单" size="48" label-size="22" margin-top="14" label-pos="bottom"></u-icon>
				</u-grid-item>
			</u-grid>
		</view>
