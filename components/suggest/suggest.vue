<template>
	<view>
		<uni-card class="box" :isFull="true" title="反馈意见" :thumbnail="contentIcon" mode="title">
			<textarea v-model="data.content" placeholder="您的反馈对我们非常重要,请在此输入."></textarea>
		</uni-card>
		<uni-card class="box" :isFull="true" title="上传图片" :thumbnail="imgListIcon" mode="title">
		...
		<uni-card class="box" :isFull="true" title="联系方式" :thumbnail="contactIcon" mode="title">
...
<script>
	export default {
		...
		watch: {
			imgList() {
				this.data.picturePath = this.imgList.map(o => o.serverPath).join(",")
			}
		},
		methods: {
			chooseImage() {
				...
				uni.chooseImage({
					...
					success: function(res) {
						res.tempFiles.forEach(o => {
							_self.api.upload("suggestion", o.path).then((data) => {
								o.serverPath = JSON.parse(data).data
