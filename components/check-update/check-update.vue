...
<script>
	export default {
		data() {
			let data = {
				url: "",
				describe: '有新版本出现~为了给您带来更好的体验，我们建议您现在进行升级！',
				is_force: false,
				show: false,
				start: false,
				installPath: ''
		...
		methods:{
			...
			update:function() {
				...
				plus.runtime.getProperty(plus.runtime.appid, function(res){
					console.log(res)
					self.api.getLatestVersion({ appid: res.appid, currentAppVersion: res.version }).then(r => {
						if (!r) return;
						if (res.version != self.version) { 
							self.url = r.downloadUrl
							self.versionShow();
							return;
						}
					})
